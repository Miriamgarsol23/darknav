...
def export_to_onnx(model, out_path):
    """Export to ONNX using the legacy API (torch.onnx.export with dynamo=False)."""
    model.eval()
    dummy = torch.zeros(1, 1, 128, 128)
    out_path = Path(out_path)

    try:
        # Try legacy API first (works without onnxscript)
        torch.onnx.export(
            model,
            dummy,
            str(out_path),
        ...
        print(f"  Exported ONNX: {out_path.name} ({size_mb:.1f} MB)")
    except Exception as e:
        print(f"  ONNX export failed: {e}")
        print("  Skipping ONNX export. Install onnxscript to enable: pip install onnxscript")
...
def main():
    ...
    conditions = [
        ('condition_A', 'scratch',  runs_dir / 'condition_A' / 'condition_A_best.pth'),
        ('condition_B', 'imagenet', runs_dir / 'condition_B' / 'condition_B_best.pth'),
        ('condition_C', 'scratch',  runs_dir / 'condition_C' / 'condition_C_best.pth'),
    ]

    all_results = []
    for name, mode, ckpt_path in conditions:
    ...
    print(f"{'Condition':<15} {'IoU':>6} {'Prec':>6} {'Rec':>6} {'F1':>6} {'CPU ms':>8}")
    print("-" * 52)
